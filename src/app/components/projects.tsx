...
const ExternalLink = React.memo(() => (
    <svg className={styles.external} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
export function Projects() {
    const projects = [
        { className: "waf", name: "Web Application Firewall", link: "https://github.com/dayeya/FinalWall" },
        { className: "asm", name: "Assembly 2D Golf Simulator", link: "https://github.com/dayeya/Mini_Golf" }
    ]; 
    return (
        <div className={styles.projects}>
            <div className={styles.bio_header}>Recent Work</div>
            <div className={styles.projects_con}>
                {projects.map((project, index) => (
                    <Project key={index} className={project.className} name={project.name} link={project.link} />
                ))}
            </div>
        </div>
    );
